...
def sample(self, N, convert_to_numpy=False, suppress_noise=True):
    with torch.no_grad():
        ...
        if not suppress_noise:
            x += np.sqrt(self.var) * torch.randn(N, self.num_var,
                                                 device=device)
        if convert_to_numpy:
            z = z.cpu().numpy()
            x = x.cpu().numpy()
...
decoder = MLPDecoder(train_x.shape[1], num_latent, num_neurons,
                     var=var_x).to(device)
...
optimizer = torch.optim.Adam(list(decoder.parameters()) +
                             list(encoder.parameters()), lr=0.0001)
ELBO_history = []
for epoch in range(num_epochs):
    ...
    for batch_count, idx in enumerate(idx_batches):
        ...
        log_p = -0.5 * torch.sum(np.log(decoder.var * 2 * np.pi) + d2 /
                                 decoder.var)
        KL = -0.5 * torch.sum(1 + torch.log(batch_var_z) - batch_mu_z**2 -
                              batch_var_z)
    ...
    if epoch % 5 == 0:
        with torch.no_grad():
            ...
            z_encoded = mu_z + torch.sqrt(var_z) * torch.randn(5, num_latent,
                                                               device=device)
            # decode the samples
