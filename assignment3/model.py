...
import numpy as np
import torch
from torch.nn import Conv2d, Linear, BatchNorm2d, ConvTranspose2d, ELU, Softplus, Module, Sigmoid
...
class CategoricalEncoder(Module):
    def __init__(self, n_classes: int, n_bins=50, in_channels=1, n_conv_blocks=3, n_filters=32, n_fc=2048):
        ...
        self.dist = Linear(n_fc, n_bins * n_classes)
    ...
    def forward(self, x):
        ...
        for layer in self.conv_head:
            res = layer(res)
        res = res.flatten(1)
        dist = self.dist(res).view(-1, self.n_bins, self.n_classes)
        return dist


class CategoricalDecoder(Module):
    def __init__(self, n_pixels: int, n_classes: int, n_bins=50, n_deconv_blocks=3, in_channels=128, n_fc=2048, var=0.05):
        ...
        self.var = var
        self.filter_size = int(sqrt(n_fc / in_channels))
        self.dense = Linear(n_classes * n_bins, n_fc)
    ...
    def forward(self, z):
        ...
        assert z.shape[1:] == (self.n_bins, self.n_classes)
    ...
    def sample(self, n_samples, device, convert_to_numpy=False, suppress_noise=True):
        with torch.no_grad():
            z = torch.randn(n_samples, self.n_bins, self.n_classes, device=device)
            mu = self.forward(z)
            x = mu
            # The conditional VAE distribution is an isotropic Gaussian, hence we just add noise when sampling it
            # Suppress this for images
...
class BernoulliEncoder(Module):
    def __init__(self, n_latent: int, in_channels=1, n_conv_blocks=3, n_filters=32, n_fc=2048):
        ...
        for i in range(n_conv_blocks):
            ...
            n_filters *= 2
        self.conv_head = torch.nn.ModuleList(conv_head)
        self.latent = Linear(n_fc, n_latent)
...
class BernoulliDecoder(Module):
    def __init__(self, n_latent: int, n_deconv_blocks=3, in_channels=128, n_fc=2048, var=0.05):
        ...
        for i in range(n_deconv_blocks):
            ...
            in_channels = out_channels
        self.de_conv_head = torch.nn.ModuleList(de_conv_head)
        self.mu = torch.nn.Linear(784, 784)
        self.out_act = Sigmoid()

    def forward(self, z):
        assert len(z.shape) == 2  # [batch_size, n_latent]
    ...
    def sample(self, n_samples: int, device: str):
        with torch.no_grad():
            z = torch.randn(n_samples, self.n_latent, device=device)
            z = gumbel_softmax(z)
            x = self.forward(z)
