...
if __name__ == "__main__":
    ...
    print(f'Correlation between train and test log parameter: : {np.mean(np.array([np.corrcoef(log_params_train[:, i, j], log_params_test[:, i, j]) for i in range(2) for j in range(2)])[:, 1, 0])}\n')
    ...
    print(f'Correlation between train and test log probabilities: {np.corrcoef(log_probs_train, log_probs_test)[0, 1]}\n')

    binary_samples_train = np.mean(clt_train.sample(args.n_samples), axis=0)
    binary_samples_test = np.mean(clt_test.sample(args.n_samples), axis=0)
    print(f'Train sample averages: {binary_samples_train.round(3)}')
    print(f'Test sample averages: {binary_samples_test.round(3)}')
    print(f'Correlation between train and test samples: {np.corrcoef(binary_samples_train, binary_samples_test)[0, 1]}\n')
